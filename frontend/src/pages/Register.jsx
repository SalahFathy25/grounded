...
export default function Register() {
  ...
  const field = (name, label, type, value, setter, Icon, opts = {}) => (
    <div>
      ...
      {errors[name] && <p role="alert" className="mt-1.5 text-xs font-medium text-danger">{errors[name]}</p>}
    </div>
  )
  ...
  return (
    <div className="mx-auto flex min-h-[65vh] max-w-md flex-col justify-center px-4 py-12">
      <div className="card animate-fade-up p-7 shadow-pop sm:p-9">
        ...
        <p className="mt-1 text-sm text-muted">{t('register.sub')}</p>
        ...
        <form onSubmit={submit} className="mt-7 space-y-5" noValidate>
          ...
          <div>
            ...
            {errors.password && <p role="alert" className="mt-1.5 text-xs font-medium text-danger">{errors.password}</p>}
          </div>
          ...
          {errors.form && (
            <p role="alert" className="rounded-lg bg-red-50 px-4 py-3 text-sm font-medium text-red-700">{errors.form}</p>
          )}
